...
public class CareTasksController : BaseController
{
    ...
    public async Task<ActionResult<ApiResponse<PagedResultDto<BatchCareTaskDto>>>> GetCareTasks([FromQuery] string? status, [FromQuery] string? searchTerm, [FromQuery] int page = 1, [FromQuery] int pageSize = 10)
    {
        var query = new GetBatchCareTasksQuery { Status = status, SearchTerm = searchTerm, Page = page, PageSize = pageSize };
